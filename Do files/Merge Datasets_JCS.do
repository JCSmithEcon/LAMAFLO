...
MERGE DATASETS.DO
	...
	UPDATES:
	* PRECEDENCE CHANGED FROM EDUCATION>LIFE>ANNUAL.
	* AS IN LW, BHPS TAKES PRECEDENCE OVER UKHLS.
...
local j=0
foreach i in Annual Life Education{										// ORDER CHANGED.
	local j=`j'+1
...
replace Start_MY = Birth_MY if Start_MY<Birth_MY & !missing(Start_MY,Birth_MY)	// CORRECT 1 CASE IN EUI DATA RESULTING FROM IMPUTATION OF BIRTH MONTH AS JUNE
