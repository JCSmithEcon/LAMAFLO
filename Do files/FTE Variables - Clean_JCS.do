...
FTE VARIABLES - CLEAN.DO
	...
	IN WHICH PARTICIPANT LEFT FULL TIME EDUCATION FOR FIRST TIME OR LATEST AGE AT WHICH
	PARTICIPANT HAD NOT LEFT FULL-TIME EDUCATION. 
	
	UPDATES APPLIED WHEN PRIORITY IS GIVEN TO WORK HISTORIES:
	* DOES NOT DROP SOME END DATES BASED ON AGE CRITERIA AND COMPARISON WITH OTHER VARIABLES.
	* DROPS DATE STILL IN EDUCATION IF DATE LEFT EDUCATION EXISTS.
	
	"$work_educ"=="educ" VERSION:
	* USES LW ORIGINAL CODE WHICH USES "Several decision rules, but basically, assume no gap in education where FE before age 24. Otherwise, use school if finished before age 19."
			
	* LW VERSION USES "$work_educ"=="educ" OPTION AND LW ORIGINAL "FTE Variables - Clean.do" (SEE BOTTOM HALF OF THIS FILE).

********************************************************************************
*/

if "$work_educ"=="work" | "$work_educ"=="WORK" {						// THIS CODE IN {} IS USED IF WORK HISTORIES ARE PRIORITISED. THERE IS ONLY A MINOR CHANGE TO CODE BUT NEVERTHELESS LW CODE IS USED IN THE BOTTOM HALF OF THIS FILE IF EDUCATION HISTORIES ARE PRIORITISED.
	...
	*/
	...
	foreach i of varlist *FIN_MY *NO_MY{
		...
		prog_scrubvars `k'												// THIS PROGRAM OVERWRITES INFORMATION IN SOURCE, QUAL AND WAVE
		}	

	/*
	2. Choose values for further education, school and life history from different waves and collection methods
	*/
	// USE ONE VALUE FROM EACH COLLECTION METHOD (DATA SOURCE)
	foreach i in S F{
		foreach j of numlist 1 2 5 6{									// 1,2,5,6.
			di "`i'_`j':"
	...
	// KEEP ONE OBSERVATION FOR EACH OF LH, S & F
	foreach i in LH S F{
	...
	replace LH_IN_MY=. if !missing(LH_FIN_MY)										// THIS DELETES IN AND NO DATES IF FIN DATES ARE AVAILABLE.
	...
	prog_makeage S_IN_MY S_FIN_MY S_NO_MY
	//table S_FIN_MY_Age S_IN_MY_Age												// (DISPLAYED) TABLES COMMENTED OUT.
	...
	prog_makeage F_IN_MY F_FIN_MY F_NO_MY
	...
	//replace F_FIN_MY=. if F_FIN_MY_Age<=23 & F_NO_MY<F_FIN_MY						// THIS DROPS END DATE IF AGE AT END DATE < 23 AND NO EDUCATION REPORTED AT AN EARLIER DATE. NOTE ALTERATION.
	replace F_NO_MY=. if F_FIN_MY_Age<=23 & !missing(F_FIN_MY)
	//replace F_FIN_MY=. if F_NO_MY_Age<F_FIN_MY_Age & F_NO_MY_Age>=19				// THIS DROPS END DATE IF AGE OF NO EDUCATION IS LESS THAN AGE AT END DATE AND AGE AT NO EDUCATION IS 19 OR OLDER. NOTE ALTERATION.
	...
	// CHECK THAT ONE VALUE FROM IN, FIN AND NO HAS BEEN SELECTED.
	local j=0
	foreach i in LH S F{
		count if !missing(`i'_IN_MY,`i'_FIN_MY)
		local j=max(`j',`r(N)')
		display in red "`r(N)' OBS HAVE BOTH `i'_IN_MY AND `i'_FIN_MY"					// LINE ADDED.
		count if !missing(`i'_IN_MY,`i'_NO_MY)
		local j=max(`j',`r(N)')
		display in red "`r(N)' OBS HAVE BOTH `i'_IN_MY AND `i'_NO_MY"					// LINE ADDED.
		count if !missing(`i'_FIN_MY,`i'_NO_MY)
		local j=max(`j',`r(N)')
		display in red "`r(N)' OBS HAVE BOTH `i'_FIN_MY AND `i'_NO_MY"					// LINE ADDED.
		}
	//browse if !missing(S_FIN_MY,S_IN_MY)												// THESE 15 LINES ADDED.
	//browse if !missing(F_FIN_MY,F_IN_MY)
	replace S_IN_MY=. if !missing(S_FIN_MY,S_IN_MY)
	replace F_IN_MY=. if !missing(F_FIN_MY,F_IN_MY)
	local j=0
	foreach i in LH S F{
		count if !missing(`i'_IN_MY,`i'_FIN_MY)
		local j=max(`j',`r(N)')
		display in red "`r(N)' OBS HAVE BOTH `i'_IN_MY AND `i'_FIN_MY"
		count if !missing(`i'_IN_MY,`i'_NO_MY)
		local j=max(`j',`r(N)')
		display in red "`r(N)' OBS HAVE BOTH `i'_IN_MY AND `i'_NO_MY"
		count if !missing(`i'_FIN_MY,`i'_NO_MY)
		local j=max(`j',`r(N)')
		display in red "`r(N)' OBS HAVE BOTH `i'_FIN_MY AND `i'_NO_MY"
		}
	di in red "All should be equal to zero."											// THIS LINE MOVED OUT OF LOOP TO DISPLAY WARNING FOR ALL CASES.
	if `j'>0{
		STOP
		}	
...
else if "$work_educ"=="educ" | "$work_educ"=="EDUC" {						// USES LW ORIGINAL CODE (VERY SIMILAR TO THE ABOVE) IF EDUCATION HISTORIES ARE PRIORITISED.
